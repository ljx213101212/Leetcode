...
std::vector<int> szIconId = { IDI_TEST01 , IDI_TEST02,IDI_TEST03,IDI_TEST04 ,IDI_TEST05,IDI_TEST06, IDI_TEST07,  IDI_TEST08, IDI_TEST09, IDI_TEST10};
int szIconIdx = 0;
...
BOOL ChangeSysTrayIcon(int iconId) {

	HICON hMainIcon;
	HINSTANCE hInstance = hInst;
	hMainIcon = LoadIcon(hInstance, (LPCTSTR)MAKEINTRESOURCE(iconId));
	nidApp.hIcon = hMainIcon; // handle of the Icon to be displayed, obtained from LoadIcon 
	//nidApp.uCallbackMessage = WM_USER_SHELLICON;
	//LoadString(hInstance, IDS_APPTOOLTIP, nidApp.szTip, MAX_LOADSTRING);
	...
	case WM_USER_SHELLICON: 
		...
		{   
			case WM_RBUTTONDOWN: 
				...
				InsertMenu(hPopMenu,0xFFFFFFFF,uFlag,IDM_TEST2,_T("Toggle Icon")); // Test 2
	...
	case WM_COMMAND:
		...
		switch (wmId)
		{
			...
			case IDM_TEST1:
				...
				break;
			case IDM_TEST2:
				//MessageBox(NULL,_T("Toggle Icon"),_T("Toggle Icon 2"),MB_OK);
				if (szIconIdx >= szIconId.size()) {
					szIconIdx = 0;
				}
				ChangeSysTrayIcon(szIconId[szIconIdx]);
				szIconIdx++;
				break;
			case IDM_DISABLE:
				bDisable = TRUE;
